...
(function () {
  ...
  mpPlay.addEventListener('click', () => {
    ...
    if (isPlaying) {
      nativeAudio.pause();
      if (synced) broadcastSync({ action: 'pause' });
    } else {
      nativeAudio.play().catch(() => {});
  ...
  /* FIX: sync seek position periodically when playing */
  let syncInterval = null;
  nativeAudio.addEventListener('play', () => {
    isPlaying = true; updatePlayBtn();
    if (synced) {
      clearInterval(syncInterval);
      // Broadcast current time every 5 seconds to keep in sync
      syncInterval = setInterval(() => {
        if (synced && !nativeAudio.paused)
  ...
  window._zxReceiveSync = function (data) {
    // FIX: receive sync even if this client hasn't pressed Sync yet
    // so the other person's actions are always mirrored
    ...
    // FIX: also handle seek + play/pause sync for native audio
    if (data.action === 'seek' && activeType === 'audio') {
      nativeAudio.currentTime = data.time || 0;
    }
    if (data.action === 'pause' && activeType === 'audio') {
      nativeAudio.pause();
    }
    if (data.action === 'play' && activeType === 'audio') {
      nativeAudio.currentTime = data.time || 0;
      nativeAudio.play().catch(()=>{});
    }
  };
