/* ═══════════════════════════════════════════════════════════
   ZEROX HUB — player.js (100% FULL CODE - EXPANDED FORMAT)
...
(function () {
  ...
  if (closeHandle) {
      ...
      closeHandle.addEventListener('touchmove', (e) => { 
          if(isPanelOpen && (startY - e.touches[0].clientY) > 15) {
              closePanel(); 
          }
      }, {passive: true});
  ...
  /* ── 💥 TAB 2: M-ZIX ENGINE (JIOSAAVN MULTI-MIRROR API) 💥 ── */
          ...
          try {
              ...
              items.forEach(song => {
                  ...
                  if(song.image && Array.isArray(song.image)) {
                      thumb = song.image.slice(-1)[0]?.url || song.image[0]?.url;
                  }
                  
                  let audioLink = '';
                  if(song.downloadUrl && Array.isArray(song.downloadUrl)) {
                      audioLink = song.downloadUrl.slice(-1)[0]?.url || song.downloadUrl[0]?.url;
                  }
              ...
              console.log("Mirror failed, switching..."); 
          }
      }

      if(!success) { 
          spSearchResults.innerHTML = '<p class="mp-empty">All Global Servers busy. Try again.</p>'; 
      }
      spInput.value = '';
  }

  if(spSearchSongBtn) {
      spSearchSongBtn.addEventListener('click', () => searchMzixGlobal(spInput.value));
  }
  if(spSearchPlaylistBtn) {
      spSearchPlaylistBtn.addEventListener('click', () => searchMzixGlobal(spInput.value));
  }
